...
GET은 멱등성을 보장하고 캐싱이 가능하지만 보안에 취약하고, POST는 서버의 자원 상태를 변경하므로 멱등성을 보장하지 않는다.
...
|    서버 상태    | 서버의 데이터를 가져오는 데 사용되므로 서버의 상태를 변경하지 않음 -> `멱등성` 보장          | 서버로 데이터를 전송하는 데 사용되므로 서버의 상태를 변경함 -> `멱등성`을 보장하지 않음 |
...
- 이준형, 이것이 취업을 위한 백엔드 개발이다 with 자바, 한빛미디어(2024), 310-311p